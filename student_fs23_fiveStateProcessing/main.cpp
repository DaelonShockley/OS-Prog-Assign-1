...
//    processorAvailable = true;
    ...
    while(processMgmt.moreProcessesComing() || processCompleted < processList.size()  /* loop continues until process list has been completed */ )
    {
        ...
        bool ioBlock = false;
        p = processList.begin(); // pointer to the beginning of list
        while(p != processList.end() && operationComplete == false)
        {
            if(p -> state == processing) // check if in processing state
            {
                auto q = p->ioEvents.begin();
                while(q != p->ioEvents.end() && !ioBlock)
                {
                    if(q -> time == p -> processorTime) // checks if an io req happens
                    {
                        stepAction = ioRequest;
                        p -> state = blocked;
                        ioBlock = true;
                        ioModule.submitIORequest(time, p->ioEvents.front(), *p);
                    }
                    q++;
                }
                if(p -> processorTime == p -> reqProcessorTime && ioBlock == false) // checks if finished
                {
                    stepAction = complete;
                    p->state = done;
                    processCount++;
                }
                else if(!ioBlock) // if not finished, continue running
                {
        ...
        if(stepAction == noAct)
        {
            auto interrupt1 = interrupts.begin(); // new interrupt variable pointing to beginning of interrupts list
            p = processList.begin();
            while(interrupt1 != interrupts.end()) // loop through the interrupts list
            {
                while(p != processList.end()) // loops through the process list
                {
                    if(p->id == interrupt1->procID) // if the interrupt id matches the process id
                    {
                        stepAction = handleInterrupt;
                        p -> state = ready;
                        interrupts.erase(interrupt1); // erase interrupt from interrupts list
                    }
                    p++;
                }
                interrupt1++;
        ...
        if(stepAction == noAct)
        {
            ...
            while(p != processList.end() && operationComplete == false)
            {
                if(p -> state == ready)
                {
                    p -> state = processing;
                    stepAction = beginRun;
                    p -> processorTime++; // increment the processor time when process is started
                    operationComplete = true; // complete operation to break loop
                }
                p++;
            }
        }
